...
export const isAdmin = async (req, res, next) => {
  try {
    ...
    //Obtenemos el rol para el usuario que inició sesión //comprobamos que sea administrador
    const role = await Role.findById(userFound.role);
    if (!role) {
      //No se encuentra el rol del usuario
      //Retornamos error en el login
      return res.status(401).json({
        message: ["No autorizado, el rol para el usuario no está definido"],
      });
    }
    if (role.role != roleAdmin) {
      //Retornamos error en el login
      return res.status(401).json({
        message: ["El usuario no es está autorizado para esta operación"],
      });
    }
    next();
  } catch (error) {
