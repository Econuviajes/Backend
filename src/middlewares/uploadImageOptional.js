...
// Versión opcional: si no hay imagen, continúa sin error
export const uploadToCloudinaryOptional = async (req, res, next) => {
  try {
    const allowedMimes = [
      "image/jpeg",
      "image/jpg",
    ...
    upload(req, res, async (err) => {
      ...
      // Si no hay archivo, simplemente continuamos sin subir imagen
      if (!req.file) {
        req.urlImage = undefined;
        return next();
      }
